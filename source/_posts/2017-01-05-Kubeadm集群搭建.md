...
具体请参考[使用Flannel搭建docker网络](/blog/2017/01/%E4%BD%BF%E7%94%A8Flannel%E6%90%AD%E5%BB%BAdocker%E7%BD%91%E7%BB%9C.html)

如果采用flannel的话，需要去掉cni网络：
...
测试：
```bash
#kubectl run busybox --rm -ti --image=busybox -- nslookup -type=srv kubernetes
...
该方式需要认证，安装后会出现很多异常，请参考下面异常处理。

http://192.168.10.6:9090/ui
参考：[https://github.com/qianlei90/Blog/issues/28](https://github.com/qianlei90/Blog/issues/28)
因为使用kubeadm安装的集群是不带认证的，所以无法直接从https://<master ip>/ui访问，可以通过：

```bash
kubectl proxy --address 192.168.10.6 --port=9090 --accept-hosts='^*$'
