...
初始化
```bash
kubeadm init --pod-network-cidr=10.244.0.0/16 --config kubeadm-config.yml
```
...
### 异常处理
node加入后，STATUS状态为：NotReady，查看kubelet日志：
```bash
...
kubeadm join --token=67a477.959aa53030fd8444 192.168.10.6
