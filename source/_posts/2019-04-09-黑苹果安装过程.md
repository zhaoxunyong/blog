...
#declare var
rootUrl=''
m3u8File=/Developer/down/xxx.m3u8
...
while read line
do 
    if [[ ${line} == /api/* || ${line} == http://* ]]; then
        if [[ ${line} == /api/* ]]; then
            line="${rootUrl}${line}"
        fi 
        fileName="${index}.mp4"
        if [ ! -f "$fileName" ]; then 
            echo "${line}" | sed "s;^;$aria2cCmd -o \"${fileName}\" \";g" \
            | sed "s;$;\";g" | sh +x
            if [[ $? == 0 ]]; then
...
# for ((i=0; i<=291; i ++))
# do
#     fileName=`printf file-%04d.txt $i`
#     if [ ! -f "$fileName" ]; then
#         printf "${rootUrl}/file-%04d.txt\n" $i \
#         | sed "s;^;aria2c -o \"${fileName}\" \";g" \
#         | sed "s;$;\";g" | sh +x
#         if [[ $? == 0 ]]; then
#             echo "file '${fileName}'" >> $filelistFile
#         fi
...
#declare var
m3u8File=05714ecace60df265fe0f0f4df60d9fb_1.m3u8
rootUrl=
filePrifix=ts
outputFile=`echo "$m3u8File"|sed "s;m3u8;mp4;"`
filelistFile=`echo "$m3u8File"|sed "s;m3u8;txt;"`
tempFolder=`echo "$m3u8File"|sed "s;.m3u8;;"`
header="--header='Referer: https://abc.com'"
userAgent="Mozilla/5.0 (Macintosh\; Intel Mac OS X 10_14_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.103 Safari/537.36"
# aria2cCmd="aria2c $header -U '$userAgent'"
aria2cCmd="aria2c -U '$userAgent'"
if [ ! -d "$tempFolder" ]; then
    mkdir -p "$tempFolder"
fi
...
#checkout either the finally file existed
if [ -f "$outputFile" ]; then
    echo "$outputFile already exist!"
    exit -1
fi

echo -n "" > $filelistFile

cat $m3u8File | sed '/^$/d' | sed '/^#.*/d' | sed '/^#.*/d' | sed "s;\(.*\)\/\(.*\.$filePrifix\)\(.*\)$;$aria2cCmd -c -o '$tempFolder/\2' '${rootUrl}\1/\2\3';g" | parallel -j 10
sleep 2
ls $tempFolder/* | sort -k 1 -V | sed "s;^;file ';"|sed "s;$;';" > $filelistFile
sleep 1
if [ -f "$filelistFile" ]; then
    echo "ffmpeg -f concat -i $filelistFile -c copy $outputFile"
fi
```
...
tempFile=aaaa
outputFile=${currPwd}/${tempFile}.mp4
tempFolder=/tmp/${tempFile}
[[ ! -e $tempFolder ]] && mkdir -p $tempFolder
echo "The temporary folder is: ${tempFolder}"
cd $tempFolder

youtube-dl -i -f best $url

ls *.flv | sort -t '.' -k 1 -V \
 | LC_CTYPE=C sed "s;^;file '$PWD/;g" \
 | LC_CTYPE=C sed "s;$;';g" \
 > ${tempFile}.list

echo "The output file is: ${outputFile}"
ffmpeg -f concat -safe 0 -i ${tempFile}.list -c copy ${outputFile}
rm -fr $tempFolder/*
cd $currPwd
```
...
```
ffmpeg -i xxx.mp4 -codec copy -bsf:v h264_mp4toannexb xxx.mp4.ts
ffmpeg -i xxx.mp4.ts -c copy -map 0 -f segment -segment_list playlist.m3u8 -segment_time 3 xxx%03d.ts
```
