...
参考以下文件：[devcontainer.zip](/files/devcontainer.zip)

注意：

如果有变动，需要执行"Remote-Containers: Rebuild Container"。另外不支持远程docker(2375端口)方式打开本地的目录(因为无法远程mount目录)，但可以通过"Remote-Containers: Clone Repository in Container Volume"的方式进行。
