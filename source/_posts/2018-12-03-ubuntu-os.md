...
sudo apt install ./fcitx-baidupinyin.deb

#解决中文输入乱码的问题
#https://blog.csdn.net/qq_37435462/article/details/139027276
sudo apt install fcitx-libpinyin
#不出意外的话你就可以看到跳出了百度输入法的配置菜单
```
...
sudo mkdir -p /usr/share/fonts/truetype/mac-fonts/
sudo unzip mac-fonts.zip -d /usr/share/fonts/truetype/mac-fonts/
sudo chmod -R 777  /usr/share/fonts/truetype/mac-fonts
