...
#将A机器的~/.ssh/id_rsa.pub内容追加到B机器的~/.ssh/authorized_keys：
#先登录到B机器后执行：
echo "<替换为A机器的~/.ssh/id_rsa.pub内容>" >> ~/.ssh/authorized_keys
```
...
3.写入、修改并保存配置
```
# ThinkBook 16 G7+ IAH models come with pressure pads that were not
...
```bash
sudo apt install xserver-xorg-input-libinput xserver-xorg-input-evdev xserver-xorg-input-mouse xserver-xorg-input-synaptics
xinput set-prop 14 "Synaptics Finger" 0 0 0
```
