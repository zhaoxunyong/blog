...
function uploadApp() {
  ...
  echo "Uploading app for $platform is successful..."
  return 0
}
...
function buildIos() {
  ...
  if [[ "$ENV" == "prod" ]]; then
    ...
    echo "Uploading to app store, it may make a few minutes, please wait..."
    ...
    if [[ $? != 0 ]]; then
      ...
      echo "Uploading to app store successfully!"
    fi
  else
    xcodebuild -exportArchive -archivePath "${output}/${APP_NAME}-${BUILD_TYPE}.xcarchive" \
      -exportPath "${output}/${APP_NAME}-${BUILD_TYPE}.ipa" \
