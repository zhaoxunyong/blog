...
token=`curl -s -X POST "${serverUrl}/api/user/login" -H "accept: application/json" -H "Authorization: aaa" -H "content-type: application/json" -d "{ \"username\": \"zhaoxunyong\", \"password\": \"xxx\"}" | python -mjson.tool | grep token | awk -F'[:"]' '{print $5}'`
...
function uploadApp() {
  ...
  if [[ $uploadResult == *\"message\":\"当前版本已存在\"* ]]; then
    ...
    deleteMaxApp $platform
    echo "Uploading app again for $platform $filePath..."
    uploadApp $platform $filePath
    # If not exit(), the following script will excute repeate!
    exit 0
  fi
