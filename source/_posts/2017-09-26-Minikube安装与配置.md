...
```bash
ssh -q -N -f -D 0.0.0.0:9999 vagrant@47.89.11.117
```

也可以通过privoxy将SOCKS5转换为http服务，修改/etc/privoxy/config配置：
...
function proxy_on() {
    export no_proxy="localhost,127.0.0.1,localaddress,.localdomain.com,xwalletsit.zerofinance.hk"
...
可以在chrome通过SwitchyOmega设置SOCKS5的9999的端口、或者http的8888端口。
...
```cmd
minikube.exe start --vm-driver="hyperv" --hyperv-virtual-switch="OuterNetwork" --docker-env HTTP_PROXY=http://192.168.0.102:1080 --docker-env HTTPS_PROXY=http://192.168.0.102:1080
```
