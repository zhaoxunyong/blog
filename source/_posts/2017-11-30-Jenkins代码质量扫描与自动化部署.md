...
// 需要过滤掉的邮件地址
def excludeEmails = ['a@a.com', 'b@b.com']
...
for(def cause in buildCauses) {
    try {
        if(cause.shortDescription.indexOf("Started by upstream") != -1 || cause.shortDescription.indexOf("Started by downstream") != -1) {
            cancel = true
        } else if(cause.shortDescription.indexOf("Started by timer") != -1) {
...
注意：
Git parameter只作一些简单的过滤，如果想实现复杂的过渡的话，可以用Groovy脚本（通过Extensible Choice Plugin实现）：
...
def command = " git ls-remote --tags -h $gitURL"
def proc = command.execute()
proc.waitFor()
def branches = []
branches = proc.in.text.readLines().collect().findAll {
  it ==~ /\d+.*[release|test]-\d+/
}.collect {
