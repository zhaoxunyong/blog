...
if test $2 = "connected"; then
  ...
  cru a check-xray "*/1 * * * * /jffs/scripts/xray-check.sh > /dev/null"

  #check date files every date
  cru a check-dat "0 1 * * * /jffs/scripts/xray-update.sh > /dev/null"

  #check dnspod on 00:00 of every day
  cru a ddns-start "0 0 * * * /jffs/scripts/ddns-start > /dev/null"

  sleep 5
  echo "" > /jffs/xray/xray-status.log
...
0)  echo "Restarting xray:     $(date)" >> /jffs/xray/xray-status.log
    ...
    #iptables
    /jffs/scripts/xray-iptables.sh >> /jffs/xray/xray-status.log
    ;;
1)  # all ok
    #iptables
    /jffs/scripts/xray-iptables.sh >> /jffs/xray/xray-status.log
