...
if test $2 = "connected"; then
  #check xray every 5 minute
  cru a check-xray "*/5 * * * * /jffs/scripts/xray-check.sh > /dev/null"

  #check dnspod on 00:00 of every day
  cru a ddns-start "0 0 * * * /jffs/scripts/ddns-start > /dev/null"

  #/jffs/scripts/ipset.sh
  #/jffs/scripts/startVPN.sh > /jffs/xray/startVPN.log &

  sleep 5
  /jffs/scripts/xray-check.sh
...
0)  echo "Restarting xray:     $(date)" >> /jffs/xray/xray-status.log
    nohup /jffs/xray/xray -c /jffs/xray/config.json >> /jffs/xray/xray-status.log 2>&1 &
    #iptables
    /jffs/scripts/router-iptables.sh >> /jffs/xray/xray-status.log
    ;;
1)  # all ok
    #iptables
    /jffs/scripts/router-iptables.sh >> /jffs/xray/xray-status.log
    ;;
*)  echo "Removed double xray: $(date)" >> /jffs/xray/xray-status.log
...
if [ $? -ne 0 ]; then
    ...
    #直连中国的IP
    #iptables -t nat -A TROJAN -m set --match-set china_ip dst -j RETURN
    ...
    iptables -t mangle -A TROJAN -p udp --dport 53 -j TPROXY --on-port 1080 --tproxy-mark 0x01/0x01
