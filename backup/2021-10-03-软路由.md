...
VPN IPv4-Network-Address：192.168.0.1
...
if [ $? == 0 ]; then
  echo "$user is existing, resting the password..."
  ocpasswd -c /etc/ocserv/ocpasswordfile -d $user
  $0 $user
else
  ocpasswd -c /etc/ocserv/ocpasswordfile $user
  /etc/init.d/ocserv restart
  echo "Copying file to remote..."
  scp -p /etc/ocserv/ocpasswordfile root@192.168.100.142:/etc/ocserv/ocpasswordfile
...
do
  ...
  if [ $? == 0 ]; then
    echo "$user is existing, don't need to add again."
  else
...
listen test1
    bind 0.0.0.0:4443
    mode tcp
    server s1 192.168.100.141:7443
    server s2 192.168.100.142:7443
```
